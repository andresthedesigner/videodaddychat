 ...
 */
export async function checkUsage(supabase: SupabaseClientType, userId: string) {
...
export async function checkProUsage(supabase: SupabaseClientType, userId: string) {
  const { data: userData, error: userDataError } = await supabase
    .from("users")
